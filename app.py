...
datas = pd.read_excel('static/market_price.xlsx')
...
def detect(save_img=False):
    ...
    t0 = time.time()
    if not cam:
        for path, img, im0s, vid_cap in dataset:  # 무한루프
            ...
            # Apply NMS
            pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms)  # list
            # [(number of detected object, 6)]  6: box coord(4), conf, class
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if len(det):  # detect 된 물체가 있으면
                    ...
                    det_cls = det[:, -1]
                    det_cls_list = [names[int(i)] for i in det_cls]
                    detected_labels = []
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or view_img:  # Add bbox to image
                            ...
                            plot_one_box(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=3)  # post processing
                ...
                # Stream results
                if view_img:
                    # 여기서의 im0 이미지를 플라스크에서 사용
                    im0 = imutils.resize(im0, width=600)
